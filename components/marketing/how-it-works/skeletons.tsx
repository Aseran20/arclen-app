...
export const ConnectYourTooklsSkeleton = () => {
  ...
  const [randomWidths, setRandomWidths] = useState<Array<{ initial: number; animate: number }>>([]);

  useEffect(() => {
    setMounted(true);
    setRandomWidth(Math.random() * 100);
    setRandomWidths([
      { initial: 20 + Math.random() * 20, animate: 70 + Math.random() * 30 },
      { initial: 20 + Math.random() * 20, animate: 70 + Math.random() * 30 },
      { initial: 20 + Math.random() * 20, animate: 70 + Math.random() * 30 },
    ]);
  }, []);
  ...
  return (
    <div className="relative flex h-full w-full items-center justify-between">
      ...
      >
        ...
        <div className="mt-2 flex flex-col">
          {randomWidths.map((widths, index) => (
            <motion.div
              key={`width-bar-right-${index}`}
              initial={{
                width: `${widths.initial}%`,
              }}
              animate={{
                width: `${widths.animate}%`,
              }}
              transition={{
                duration: 4,
                delay: index * 0.2,
