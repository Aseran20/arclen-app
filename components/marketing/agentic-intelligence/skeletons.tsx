...
export const LLMModelSelectorSkeleton = () => {
  ...
  const [sparkleValues, setSparkleValues] = useState<Array<{
    randomX: number;
    randomY: number;
    randomDelay: number;
  ...
  useEffect(() => {
    ...
    setSparkleValues(
      Array.from({ length: 8 }).map(() => ({
        ...
        randomDelay: Math.random() * 0.8,
        randomDuration: 0.5 + Math.random() * 1,
        randomScale: 0.5 + Math.random() * 0.5,
      }))
    );
  }, []);
    ...
    <motion.div className="relative mx-auto mt-20 h-full max-h-70 min-h-40 w-[85%] rounded-2xl border-t border-gray-300 bg-white p-4 shadow-2xl dark:border-neutral-700 dark:bg-neutral-800">
      ...
      {models.map((model, index) => (
        <div className="relative" key={model.name + index}>
          ...
          >
            {sparkleValues.map((sparkle, sparkleIndex) => (
              <motion.div
                key={sparkleIndex}
                initial={{
                  opacity: 0,
                ...
                viewport={{ once: true }}
                transition={{
                  duration: sparkle.randomDuration,
                  delay: index * 1 + sparkle.randomDelay,
                  ease: "easeOut",
                }}
...
export const TextToWorkflowBuilderSkeleton = () => {
  ...
  const handleSendMessage = () => {
    if (inputText.trim()) {
      const messageIndex = messageIndexRef.current % RANDOM_MESSAGES.length;
      ...
      const newMessages = [
        ...
        {
          role: "assistant",
          content: RANDOM_MESSAGES[messageIndex],
        },
      ];
      setChat(newMessages);
